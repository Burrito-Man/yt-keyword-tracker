...
- 검색 모드 2가지
  - **전체 검색 (Search API)**: 키워드로 유튜브 전체 검색 (쿼터 소모 큼)
    - "🚫 공식 채널 제외" 체크박스로 메이플스토리 IP 공식 채널(넥슨, 감마니아 등 7개, 기본 꺼짐)을 결과에서 제외 가능 — 크리에이터 콘텐츠 성과만 순수하게 보고 싶을 때 사용
...
- **채널 스냅샷** (사이드바 모드 전환): 키워드·날짜 필터 없이, 선택한 채널 프리셋의 각 채널에서 **최신 영상 N개**를 그대로 훑어보는 기능
  - 채널당 API 호출 1~2회 수준으로 매우 가벼움 (전체 27개 채널 조회에 30 unit 미만)
  - 결과는 채널 열이 포함된 표 하나로 통합 (제목·링크·게시일·조회수·분류까지 포함), CSV 다운로드 가능
  - 특정 시점 스냅샷으로 "요즘 크리에이터들이 실제로 어떤 콘텐츠를 올리고 있는지" 빠르게 훑어볼 때 사용
